...
export async function getHashedPasswordByUsername(
    username: string
): Promise<any> {
    const [rows] = await pool.query<RowDataPacket[]>(
...
export async function searchIgredientName(
...
export async function checkIngredientExists(
    ingredient_name: string
): Promise<any> {
    const [rows] = await pool.query<RowDataPacket[]>(
...
export async function hasUserAddRestrictedIngredient(
    username: string,
    ingredient_name: string
): Promise<any> {
...
export async function fetchIngredientbyUserName(
    username: string
): Promise<any[]> {
    const user_id = Math.abs(crc32.str(username));
    console.log(user_id);
    ...
    console.log(ingredient_id_arr);
    if (ingredient_id_arr.length == 0) {
        return [];
    }
    ...
    const ingredient_names = ingredient_name_arr[0].map((row) => ({
        ingredient_name: row.ingredient_name,
        // .slice(0, -1),
    }));
    return ingredient_names as any[];
}
